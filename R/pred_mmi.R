#' Cross validation to evaluate predictive performance of Xeric MMI candidate models
#'
...
#' @param log10 An argument to either log10 transform MMI or not
#'
...
pred_mmi<-function(index_value,data,log10=FALSE) {
  ########
  xer_org<-data
  ...
  if (log10) {
    xer_org$RESPONSE <- log10(xer_org$MMI_BENT_sc + 0.01)
  } else {
    xer_org$RESPONSE <- xer_org$MMI_BENT_sc
  }
  ...
  mlr.mmi <-lm(RESPONSE~., data=train_red)# use all variables in subset except log OE
  summary(mlr.mmi)


  # Predict O/E using MLR model
  pred.mlr<-predict(mlr.mmi, test_red)
  ...
  spmod.mmi<-splm(RESPONSE~ PSUMPY_SY_WS_sc+drought_mean+
                    asin_PCTIMP_WsRp100+L_NABD_NrmStorWs_ratio+
                    L_STRM_POWER+LOE_QLow_cl+LOE_Qbkf_cl+evap_index_sc+
                    W1_HAG+W1_HNOAG+asin_PCTFORGRS_CATRP100+
  ...
  ###########
  ## SP model predict
  pred.sp <- predict(spmod.mmi, newdata=test_sf)
...
RESPONSE ~ 1 + r1*LRBS_use + n1*L_NTL + su1*L_SULF + boe1*LOE_Qbkf_cl + d1*L_NABD_NrmStorWs_ratio + na1*W1_HNOAG + sp1*L_STRM_POWER + ph1*drought_mean
...
'
  ...
  fit<- sem(mymodel_v3_MMI, data=train,
            estimator="MLM")
  ...
  ## RANDOM FOREST
  set.seed(200)
  rf.mmi <- ranger(RESPONSE~., data=train_red,
                   importance = "permutation",
                   scale.permutation.importance = TRUE)

  pred.rf <-predict(rf.mmi, data = test_red)
