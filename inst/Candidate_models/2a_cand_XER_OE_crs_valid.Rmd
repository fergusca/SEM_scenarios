---
title: "a_Cand_XER_OE_mods_crs_valid"
author: "C. Emi Fergus"
date: "2023-12-20"
...
We are interested in developing models that will allow for predicting changes in O/E under hypothetical management scenarios. We will evaluate the models based on their predictive performance (RMSE & R2). There may be benefits to using modeling approaches that allow for causal inference. 
...
#    select(all_of(myvars))
 ...
 # mlr.oe <-lm(OE_SCORE~., data=xer_red)# use all variables in subset except log OE
#  summary(mlr.oe)


  # Predict O/E using MLR model
#  pred.mlr<-predict(mlr.oe, xer_red, se.fit=TRUE)#,interval="confidence"
...
vals<-pred_oe_cv(indexes = 1:10, data=xer_org,log10=FALSE)
...
write_csv(rmse_df,"C:/Users/efergus/SEMscenarios/Routput/Scenario_modeling/Candidate_models/XER_v3_RMSE_OE.csv")
...
write_csv(r2_df,"C:/Users/efergus/SEMscenarios/Routput/Scenario_modeling/Candidate_models/XER_v3_R2_OE.csv")
```
...
# Comparing Predicted vs Observed to Observed vs Predicted
Piniero et al. 2008 demonstrates that regressing observed vs predicted is the more appropriate method to evaluate slope and intercept. Slope close to 1 indicates consistency in observed and predicted values. Intercept close to 0 indicates less bias in observed and predicted.
```{r, echo=FALSE,warning=FALSE,message=FALSE}
mlr.plot_old<-ggplot(predictions,aes(x=obs, y=mlr))+ 
  stat_poly_eq(use_label(c("eq")),size=3)+
  geom_point()+
  geom_abline(intercept=0,slope=1)+
  xlim(0,1.5)+
  ylim(0,1.5)+
...
mlr.plot<-ggplot(predictions,aes(x=mlr, y=obs))+ 
  stat_poly_eq(use_label(c("eq")),size=3)+
  geom_point()+
  geom_abline(intercept=0,slope=1)+
  xlim(0,1.5)+
...
##################
# COMPARE Predicted vs Obsrv VS Obsrv vs Predicted
pred.plots<-ggarrange(mlr.plot_old,mlr.plot,ncol=2, nrow=1)

annotate_figure(pred.plots,
top=text_grob("Predicted vs observed O/E"))

```

```{r, echo=FALSE,warning=FALSE,message=FALSE}
summary(lm(obs~mlr,predictions))
# 95% confidence intervals around parameter estimates
# Take estimate +- std. error
# Intercept
paste("Intercept 95%CI =(",round((0.0657 + 0.06314),2),",",round((0.0657 - 0.06314),2),")")

paste("Slope 95%CI =(",round((0.91697 + 0.07745),2),",",round((0.91697 - 0.07745),2),")")
