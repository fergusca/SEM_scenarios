---
title: "a_cand_XER_EPT_crs_valid"
author: "C. Emi Fergus"
date: "2024-01-30"
...
We are interested in developing models that will allow for predicting changes in EPT richness under hypothetical management scenarios. We will evaluate the models based on their predictive performance (RMSE & R2). There may be benefits to using modeling approaches that allow for causal inference. 
...
index_1<-pred_ept(index_value = 1, data=xer_org,log10=FALSE)
head(index_1)
...
pred_ept_cv<-function(indexes,...){
  map(indexes,pred_ept,...)
}

##########
## Apply function to grab the predicted values for each of the candidate models - outputs as list
vals<-pred_ept_cv(indexes = 1:10, data=xer_org,log10=FALSE)
...
write_csv(rmse_df,"C:/Users/efergus/SEMscenarios/Routput/Scenario_modeling/Candidate_models/XER_v3_RMSE_EPT.csv")
```
...
write_csv(r2_df,"C:/Users/efergus/SEMscenarios/Routput/Scenario_modeling/Candidate_models/XER_v3_R2_EPT.csv")
```
